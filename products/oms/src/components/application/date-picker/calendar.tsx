...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // AriaCalendarContext's type is shared across single/multi/range calendar
    // variants, so its onChange also accepts an array of dates. This provider
    // only ever backs the single-value Calendar below, which never passes an
    // array — the cast just narrows past a union this file doesn't use.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const contextValue = { value, onChange, focusedValue, onFocusChange } as any;
